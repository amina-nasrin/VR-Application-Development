...
@export var teleport_button_action : String = "ax_button"
...
@export var player_height : float = .8: set = set_player_height
...
var teleport_path_points: Array[Vector3] = []
...
func _physics_process(delta):
	...
	if controller and controller.get_is_active() and controller.is_button_pressed(teleport_button_action):
		...
		for i in range(1,26):
			...
			teleport_path_points.append(global_target)
...
func get_teleport_path() -> Array[Vector3]:
	return teleport_path_points
...
#	if capsule:
#		player_material = capsule.get_surface_override_material(0)
